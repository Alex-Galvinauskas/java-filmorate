...
public class DirectorDbStorageTest {
    ...
    @Test
    void getAll_shouldReturnListOfDirectors() {
        ...
        when(jdbcTemplate.query(eq("SELECT * FROM directors ORDER BY name"),
                any(RowMapper.class)))
                .thenReturn(List.of(director1, director2));

        List<Director> result = directorDbStorage.getAll();

        assertEquals(2, result.size());
        assertEquals("Christopher Nolan", result.get(0).getName());
        assertEquals("Quentin Tarantino", result.get(1).getName());
        verify(jdbcTemplate, times(1))
                .query(eq("SELECT * FROM directors ORDER BY name"),
                        any(RowMapper.class));
    }

    @Test
    void update_shouldUpdateDirectorWhenExists() {
        Director updatedDirector = Director.builder()
                .id(1L)
                .name("Christopher Nolan Updated")
                .build();

        when(jdbcTemplate.update(eq("UPDATE directors SET name = ? WHERE id = ?"),
                eq("Christopher Nolan Updated"), eq(1L)))
                .thenReturn(1);

        Director result = directorDbStorage.update(updatedDirector);

        assertNotNull(result);
        assertEquals(1L, result.getId());
        assertEquals("Christopher Nolan Updated", result.getName());
        verify(jdbcTemplate, times(1))
                .update(eq("UPDATE directors SET name = ? WHERE id = ?"),
                        eq("Christopher Nolan Updated"), eq(1L));
    }

    @Test
    void update_shouldThrowNotFoundExceptionWhenDirectorNotExists() {
        Director nonExistentDirector = Director.builder()
                .id(999L)
                .name("Non Existent")
                .build();

        when(jdbcTemplate.update(eq("UPDATE directors SET name = ? WHERE id = ?"),
                eq("Non Existent"), eq(999L)))
                .thenReturn(0);

        NotFoundException exception = assertThrows(NotFoundException.class,
                () -> directorDbStorage.update(nonExistentDirector));
